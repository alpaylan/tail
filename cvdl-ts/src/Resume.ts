...
export class Resume {
	...
	static fromJson(resume: unknown): Resume {
		...
		return new Resume(
			...
			(resume.sections as unknown[]).map((section) =>
				ResumeSection.fromJson(section),
			),
		);
	}
...
export class ResumeSection {
	...
	static fromJson(json: unknown): ResumeSection {
		...
		if (
			...
			throw new Error(
				"ResumeSection must have a section_name, data_schema, layout_schema, and data",
			);
		}
		...
		const data = new Map(
			Object.entries(json.data as { [key: string]: ItemContent }),
		);
		// @ts-ignore
		section.data = new Map(
			[...data].map(
				([key, value]) =>
					[key, ItemContent.fromJson(value)] as [ItemName, ItemContent],
			),
		);
...
export module Item {
	export function fromJson(json: unknown): Item {
		...
		const item = {
			...
			fields: new Map(
				Object.entries(json.fields as { [key: string]: ItemContent }),
			),
		};
		return item;
	}
		...
		if (typeof json === "object" && "tag" in json && "value" in json) {
			switch (json.tag) {
				...
				case "List":
					return {
						tag: "List",
						value: (json.value as ItemContent[]).map(fromJson),
					};
				case "Url":
					return {
						tag: "Url",
						value: {
							url: (json.value as { url: string; text: string }).url,
							text: (json.value as { url: string; text: string }).text,
						},
					};
			}
			...
			return {
				tag: "Url",
				value: { url: json.url as string, text: json.text as string },
			};
		}

		throw new Error(
			`Invalid ItemContent(${JSON.stringify(json)}): ItemContent must be a string, an array, or an object`,
		);
	}
