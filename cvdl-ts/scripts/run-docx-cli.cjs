...
if (compile.status && compile.status !== 0) {
	console.warn("TypeScript reported errors, but emitted JS is available. Running docx CLI anyway.");
}

const run = spawnSync(process.execPath, [docxCliPath, ...process.argv.slice(2)], {
	cwd,
	stdio: "inherit",
});
