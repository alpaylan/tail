		...
		>
			...
			<div>
				<button
					className="bordered"
					onClick={(e) => {
						dispatch!({ type: "delete-item", section: section, item: item });
						e.stopPropagation();
					}}
				>
					&#x232B;
...
const SectionItem = ({
	...
	item,
	itemContent,
}: { section: string; item: number; itemContent: ItemProps }) => {
	const [fields, setFields] = useState<ItemProps>(itemContent);
	const state = useContext(EditorContext);
	const dispatch = useContext(DocumentDispatchContext);
	const editorPath = state?.editorPath;
	const showAll =
		(editorPath?.tag === "item" || editorPath?.tag === "field") &&
		editorPath.section === section &&
		editorPath.item === item;
	const toggleShowAll = () => {
		if (showAll) {
			dispatch!({
				type: "set-editor-path",
				path: { tag: "section", section: section },
			});
		} else {
			dispatch!({
				type: "set-editor-path",
				path: { tag: "item", section: section, item: item },
			});
		}
	};
		...
		>
			{showAll ? (
				...
				<ItemHeader
					...
					copy={() =>
						dispatch!({ type: "copy-item", section: section, item: item })
					}
				/>
			)}
		</div>
