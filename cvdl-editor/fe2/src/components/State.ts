...
export const DocumentReducer = (state: EditorState, action_: EditorAction) => {
				...
				if (action.item !== -1) {
					...
					if (!undoing) {
						editHistory.push({
							...
							item: action.item,
							field: action.field,
							value: item.fields.get(action.field)!,
						});
					}
					...
					if (!undoing) {
						editHistory.push({
							...
							item: action.item,
							field: action.field,
							value: newSection.data.get(action.field)!,
						});
					}
		...
		newState.sections = resume.sections.map((section) => {
			...
			if (section.section_name === action.section) {
				if (
					...
					(action.item >= newSection.items.length &&
	...
	if (action.type === "delete-section") {
		newState.sections = resume.sections.filter(
